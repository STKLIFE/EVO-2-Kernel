	...
	if (enable) {
		...
		if (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_HEADSETL] == false
			...
			if (mIsExtSPKUse) {
				...
				#ifdef CONFIG_MTK_EXTERNAL_AUDIO_PA_HPR
				Ana_Set_Reg(AUDTOP_CON4, 0x005C, 0xffff); //modified by cheguosheng 0x005C 0x003C
        #else
				Ana_Set_Reg(AUDTOP_CON4, 0x003C, 0xffff); //modified by cheguosheng 0x005C 0x003C
				#endif
			} else {
...
static void AudDrv_GPIO_EXTAMP_Select(bool status)
{
	if(status)
	{
		mt_set_gpio_mode(34, GPIO_MODE_00); //GPIO34: DPI_D3, mode 0 
		mt_set_gpio_dir(34, GPIO_DIR_OUT);	/* output */
		mt_set_gpio_out(34, GPIO_OUT_ONE);/* high enable */
		udelay(500);	
	}
	else
	{
		mt_set_gpio_mode(34, GPIO_MODE_00); //GPIO34: DPI_D3, mode 0 
		mt_set_gpio_dir(34, GPIO_DIR_OUT);	/* output */
		mt_set_gpio_out(34, GPIO_OUT_ZERO);	/* low disbale */
		udelay(500);	
	}
}
#endif

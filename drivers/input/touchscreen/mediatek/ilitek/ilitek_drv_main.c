...
s32 DrvEnableProximity(void)
{
	...
	g_EnableTpProximity = 1; //modify esky_liml
...
if (g_FaceClosingTp==0)
	return 0;
else
  if(g_FaceClosingTp==1)
 	return 10;
 else
				   ...
				   void* pBuffOut, int nSizeOut, int* pActualOut)
{
    ...
    DBG(&g_I2cClient->dev, "*** %s() nCommand = %d ***\n", __func__,nCommand);
...
//        tsps_assist_register_callback("msg2xxx", &DrvTpPsEnable, &DrvGetTpPsData);
	...
	if (nErr) {
		 DBG(&g_I2cClient->dev, "call ps_register_control_path() failed = %d\n", nErr);
	}
	...
	if (nErr) {
		DBG(&g_I2cClient->dev, "call ps_register_data_path() failed = %d\n", nErr);
	}
            ...
            )
            {
                ...
                if (g_EnableTpProximity == 1)
                {
...
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
                    {
                        ...
                        if (ps_report_interrupt_data(tSensorData.values[0]))
                        {
                            DBG(&g_I2cClient->dev, "call ps_report_interrupt_data() failed !\n");
                        }
                    }
#endif               
                }
...
#endif //CONFIG_PLATFORM_USE_ANDROID_SDK_6_UPWARD
{
	...
	gsl_halt_flag = 1 ;
	DBG(&g_I2cClient->dev, " resume g_EnableTpProximity = %d,gsl_halt_flag=%d\n", g_EnableTpProximity,gsl_halt_flag);
#endif //CONFIG_ENABLE_PROXIMITY_DETECTION
...
#endif //CONFIG_PLATFORM_USE_ANDROID_SDK_6_UPWARD
{
    ...
    if ((g_EnableTpProximity == 1)&&(gsl_halt_flag == 0))
    {
        DBG(&g_I2cClient->dev, "g_EnableTpProximity = %d\n", g_EnableTpProximity);
		DrvEnableProximity();
        return;

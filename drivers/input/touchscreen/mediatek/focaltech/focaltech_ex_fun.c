...
#define ctp_info_size 128

unsigned char ft_fw_version=0;
char ft_ctp_version[ctp_info_size] = { 0 };

static struct proc_dir_entry *fts_ctpinfo_entry;


static int ctp_info_read(struct seq_file *m, void *v)
{
	snprintf(ft_ctp_version, sizeof(ft_ctp_version),
						 "FT: Ver=0x%x;", ft_fw_version);  
	
	printk("[maxyu]ctp_info_read %s\n", ft_ctp_version);
	seq_printf(m, "%s\n", ft_ctp_version);
	return 0;
};

static int proc_ctpinfo_open(struct inode *inode, struct file *file)
{
	return single_open(file, ctp_info_read, NULL);
};

static const struct file_operations fts_ctpinfo_fops = {
		.owner 	= THIS_MODULE,
		.open = proc_ctpinfo_open,
		.read = seq_read,
			
};

int fts_create_ctp_info_channel(struct i2c_client * client)
{
	memset(ft_ctp_version, 0, ctp_info_size);
	
	#if (LINUX_VERSION_CODE > KERNEL_VERSION(3, 10, 0))
		fts_ctpinfo_entry = proc_create(PROC_CTP_INFO, 0777, NULL, &fts_ctpinfo_fops);		
	#else
		fts_ctpinfo_entry = create_proc_entry(PROC_CTP_INFO, 0777, NULL);
	#endif
	if (NULL == fts_ctpinfo_entry) 
	{
		dev_err(&client->dev, "Couldn't create ctpinfo entry!\n");
		
		return -ENOMEM;
	} 
	else 
	{
		dev_info(&client->dev, "Create ctpinfo entry success!\n");

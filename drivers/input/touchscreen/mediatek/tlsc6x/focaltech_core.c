...
//static u8 ft6336_psensor_data[8]={0};
...
struct ft6336_priv {
	...
	static struct ft6336_priv *ft6336_obj;
	#define APS_ERR(fmt,arg...)           							printk("<<proximity>> "fmt"\n",##arg)
	#define FTS_TPD_PROXIMITY_DEBUG(fmt,arg...) 					printk("<<proximity>> "fmt"\n",##arg)
	#define FTS_TPD_PROXIMITY_DMESG(fmt,arg...) 					printk("<<proximity>> "fmt"\n",##arg)	
#endif //luyuan
...
static int tpd_enable_ps(int enable)
{
	...
	//i2c_smbus_read_i2c_block_data(client, 0xB0, 1, &state);

	ret = fts_i2c_read_reg(fts_data->client, 0xB0,&state);
	if (ret<0) 
	{
	...
	if (enable)
	{
		...
		FTS_TPD_PROXIMITY_DEBUG("[proxi_fts]ps function is on\n");	
	}
	else
	{
		state &= 0x00;	
		tpd_proximity_flag = 0;
		FTS_TPD_PROXIMITY_DEBUG("[proxi_fts]ps function is off\n");
	}
	
	//ret = i2c_smbus_write_i2c_block_data(fts_i2c_client, 0xB0, 1, &state);
	ret = fts_i2c_write_reg(fts_data->client, 0xB0,state);
	if (ret<0) 
	{
		printk("[Focal][Touch] write value fail");
		//return ret;
	}
	FTS_TPD_PROXIMITY_DEBUG("[proxi_fts]write: 0xB0's value is 0x%02X\n", state);
	return 0;
}
...
{
	printk("ft6336_obj ps enable value = %d\n", en);
	if (en) {
		tpd_enable_ps(1);
		set_bit(2/*CMC_BIT_PS*/, &ft6336_obj->enable);
	} else {
		tpd_enable_ps(0);
		clear_bit(2/*CMC_BIT_PS*/, &ft6336_obj->enable);
	}
	return 0;
}
...
int fts_proximity_recovery(struct i2c_client *client)
{
	 ...
	 if (tpd_proximity_flag == 1)
     {
		...
		if (ret<0) 
		{
         ...
         FTS_TPD_PROXIMITY_DEBUG("proxi_fts 0xB0 state value is 1131 0x%02X\n", state);
		if(!(state&0x01))
		{
			tpd_enable_ps(1);
		}
		ret = fts_i2c_read_reg(fts_data->client, 0x01,&proximity_status);
		if (ret<0) 
		{
			//printk("[Focal][Touch] read value fail");
			//return ret;
		}
	    		FTS_TPD_PROXIMITY_DEBUG("proxi_fts 0x01 value is 1139 0x%02X\n", proximity_status);
...
static int touch_event_handler(void *unused)
{
 ...
 printk("lhh %s  line ==%d\n",__FUNCTION__,__LINE__);
        ...
        if (0 == fts_gesture_readdata(ts_data)) {
            printk("succuss to get gesture data in irq handler");
            continue;
        }
#endif
 #ifdef FTS_TPD_PROXIMITY
			 ...
			 if (tpd_proximity_flag == 1)
			 {
				...
				FTS_TPD_PROXIMITY_DEBUG("tpd_proximity_detect 1149 = %d\n", tpd_proximity_detect);
				if ((err = tpd_read_ps()))
				{
					FTS_TPD_PROXIMITY_DMESG("proxi_fts read ps data 1156: %d\n", err);	
				}
                ...
                {
                    printk("call hwmsen_get_interrupt_data fail \n");
				}
			}  
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#endif
    FTS_FUNC_ENTER();
		    FTS_INFO("tpd_probe startasdasdasd");
    ts_data = devm_kzalloc(&client->dev, sizeof(*ts_data), GFP_KERNEL);
    if (!ts_data) {
        FTS_ERROR("Failed to allocate memory for fts_data");
        return -ENOMEM;
    }
		FTS_INFO("tpd_probe startbbbbbbbbbbbbbbb");
    fts_data = ts_data;
    ...
    if (client->addr != FTS_I2C_SLAVE_ADDR) {
		...
		FTS_INFO("tpd_probe startcccccccccc");
    ...
    /* Init I2C */
    fts_i2c_init();
		FTS_INFO("tpd_probe startbbbbbbbbbbbb");
    ...
    fts_reset_proc(200);
    /* is tlsc6x ? 0:not, 1:yes  */
    g_is_telink_comp = tlsc6x_tp_dect(client);
    if(g_is_telink_comp){
        ts_data->ic_info.is_incell = 0;
        fts_reset_proc(20);
    }else{    
    ...
    FTS_INFO("tpd_probe startaaaaaaaaaaaaaaaa");
#if FTS_APK_NODE_EN
...
#endif
    FTS_INFO("tpd_probe 00000000000000");
...
#endif
		FTS_INFO("tpd_probe 11111111111");
 ...
 printk("lhh %s  line ==%d\n",__FUNCTION__,__LINE__);
    fts_proximity_init(client);
       ...
       ft6336_obj = kzalloc(sizeof(*ft6336_obj), GFP_KERNEL);
...
#endif
		FTS_INFO("tpd_probe 44444444444444");
    ...
    if (ret) {
        ...
        #ifdef FTS_CTP_INFO //added by maxyu 170628
                fts_create_ctp_info_channel(ts_data->client);
        #endif
FTS_INFO("tpd_probe start55555555555");
    ...
    if (ret) {
     ...
     #ifdef FTS_CTP_INFO //added by maxyu 170628
        fts_read_reg(client, FTS_REG_FW_VER, &ft_fw_version);
        #endif


		FTS_INFO("tpd_probe start return 0");
    tpd_load_status = 1;
...
{
    FTS_FUNC_ENTER();
    FTS_INFO("luyuan Driver version: %s", FTS_DRIVER_VERSION);
    tpd_get_dts_info();

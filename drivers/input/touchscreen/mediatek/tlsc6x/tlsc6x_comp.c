...
int tlsc6x_read_bytes_u16addr(struct i2c_client *client, u16 addr, u8 *rxbuf, u16 len)
{
    ...
    while(len > 0){
        ...
        retry = 0;
        while(fts_i2c_read(client, buffer, 2, &rxbuf[offset], msgs[1].len) < 0){
            if(retry++ == 3){
                err = -1;
                break;
            }
...
int tlsc6x_write_bytes_u16addr(struct i2c_client *client, u16 addr, u8 *txbuf, u16 len)
{
    ...
    while(len){
        ...
        retry = 0;
        while(fts_i2c_write(client, buffer, msg.len) < 0){
            if(retry++ == 3){
                err = -1;
                break;
            }
...
static int tlsc6x_download_gestlib_fast( u8* pcode, u16 len)
{
    ...
    if(0 == tlsc6x_bulk_down_check(pcode, 0x8000, 1024)){
        if(0 == tlsc6x_write_bytes_u16addr(g_tlsc6x_client, 0x8400, pcode+1024, len-1024)){	//
            ret = 0;
        }
    }
...
void tlsc6x_data_crash_deal(void)
{
    ...
    new_idx_active = -1; // give the index of file which you want update, eg. 0,1,2. if equ -1, auto search history version
    ...
    if(1 == tlsx6x_get_running_cfg(tl_buf_tmpcfg)){
        if(1 == tlsx6x_get_running_cfg(tl_buf_tmpcfg)){ // double check
            if(new_idx_active >= 0){
                ...
                if(0 == find_last_valid_burn_cfg(tl_buf_tmpcfg)){
                    u32tmp = tl_buf_tmpcfg[1];
                    u32tmp = (u32tmp<<16)|tl_buf_tmpcfg[0];
                    g_tlsc6x_cfg_ver = u32tmp&0x3ffffff;

                    new_idx_active = -1;
                    for(k=0; k<sizeof(buidIn_tls_tp_lut)/(102*2);k++){  // search the match
                        if(1 == tlsc6x_tpcfg_ver_comp(buidIn_tls_tp_lut[k])){
                            new_idx_active = k;
                            break;
                        }
                    ...
                    if(new_idx_active < 0){ // not found
                        printk("Tlsc6x:force update, no build-in version!\n");
                        if(0==tlsx6x_update_burn_cfg(tl_buf_tmpcfg)){// just burn the backup back
                            printk("Tlsc6x:use backup, force update pass!\n");
                        }else{
                            g_tlsc6x_cfg_ver = 0;
                            printk("Tlsc6x:use backup, force update fail!\n");
                        }
                    }else{  
                        if(0==tlsx6x_update_burn_cfg(buidIn_tls_tp_lut[new_idx_active])){
                            printk("Tlsc6x:auto update pass!\n");
                    ...
                    printk("Tlsc6x:no backup found!\n");
                }
            }
        }
    }
#else
    printk("Tlsc6x:force update, open TLSC_AUTO_UPGRADE please!\n");
#endif
    return;
}

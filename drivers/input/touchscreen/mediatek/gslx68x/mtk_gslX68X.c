...
#ifdef S31XX_QHD_XXX_XXX //Example
#include "mtk_gslX68X_S31XX_QHD_XXX_XXX.h" 
#elif defined(S9B_80_ZHY_B3_DRV)
#include "mtk_gslX68X_zyh_b3_hd.h" 
#elif defined(S9B_80_X8_DRV)
#include "mtk_gslX68X_xiaoxiang_x8_fwvga.h"
#elif defined(S9B_80_TENGJUN)
#include "mtk_gslX680_tengjun_fwvga.h"
#elif defined(S51_QHD_QMX_P401)
#include "mtk_gslX680_s51_qmx_p104.h"
#elif defined(S50_QHD_BP301)
//#include "mtk_gslX68X_qhd_s50_bp301.h"
#include "mtk_gslX68X_qhd_s50_bp301_yanqi.h"
#elif defined(S50_FWVGA_BP301)
#include "mtk_gslX68X_fwvga_s50_bp301.h"
#elif defined(S608_QHD_JHM_M072)
#include "mtk_gslX68X_qhd_s608_jhm_m072.h"
#elif defined(S608_QHD_JHM_M072_GESTURE)
#include "mtk_gslX68X_qhd_s608_jhm_m072_gesture.h"
#elif defined(S608_QHD_JHM_M083)
#include "mtk_gslX68X_qhd_s608_jhm_m083.h"
#elif defined(S608_QHD_JHM_M083_GESTURE)
#include "mtk_gslX68X_qhd_s608_jhm_m083_gesture.h"
#elif defined(S6_QHD_JHM_M088)
#include "mtk_gslX68X_qhd_s6_jhm_m088.h"
#elif defined(S9B_80_FWVGA_TENGJUN_O1)
#include "mtk_gslX680_fwvga_s9b_tengjun_o1.h"
#elif defined (S99_WVGA_MAILETAI_J4001)
#include "mtk_gslX680_wvga_s99_mailetai_j4001.h"
#elif defined(S9B_80_TENGJUN_O3)
#include "mtk_gslX680_tengjun_o3_fwvga.h"
#elif defined(S9B_80_TENGJUN_O5)
#include "mtk_gslX680_tengjun_o5_fwvga.h"
#elif defined(S9B_80_TENGJUN_O_QHD)
#include "mtk_gslX680_tengjun_o_qhd.h"
#elif defined(S9B_80_TENGJUN_B1_QHD480)
#include "mtk_gslX680_tengjun_B1_qhd480.h"
#elif defined(S9B_80_FW_HQ_5007)
#include "mtk_gslX680_hq_5007_fw.h"
#elif defined(S9B_YOUSHI_K01)
#include "mtk_gslX68X_fwvga_s9_youshi_k01.h"
#else
...
#define KEY_GESTURE_U 					301
#define KEY_GESTURE_UP 					296
#define KEY_GESTURE_DOWN 					297
#define KEY_GESTURE_LEFT 					298 
#define KEY_GESTURE_RIGHT 					299
#define KEY_GESTURE_O 					291
#define KEY_GESTURE_C 					288
#define KEY_GESTURE_E 						289
#define KEY_GESTURE_M 					290 
#define KEY_GESTURE_W 					294
#define KEY_GESTURE_S 					292 
#define KEY_GESTURE_V 					293
#define KEY_GESTURE_Z 						295
...
//[denqin begin] zhuguolang 20160719
//[denqin reasion]
#ifdef TPD_PROC_DEBUG
	if(1 == gsl_proc_flag)
		return;
#endif
//[denqin end] zhuguolang 20160719	
	if(i2c_lock_flag != 0)
		return;
	else
		i2c_lock_flag = 1;
...
*/
	...
	point_num = cinfo.finger_num;
		...
		if(GE_ENABLE == gsl_gesture_status && gsl_gesture_flag == 1){
			...
			print_info("Linc gsl_obtain_gesture():tmp_c=0x%x\n",tmp_c);
#if (defined(S608_QHD_JHM_M072_GESTURE)||defined(S608_QHD_JHM_M083_GESTURE))
	switch(tmp_c){
			case (int)'*':	
				key_data = KEY_GESTURE_U;
				break; /* double click */						
			}
#else
	switch(tmp_c){
			case (int)0xa1fa:
				key_data = KEY_GESTURE_RIGHT;
				break;/* right */			
			case (int)0xa1fb:	
				key_data = KEY_GESTURE_LEFT;
				break;	/* left */
			case (int)0xa1fc:	
				key_data = KEY_GESTURE_UP;
				break;/* up */
			case (int)0xa1fd:
				key_data = KEY_GESTURE_DOWN;
				break;/* down */

			case (int)'C':
				key_data = KEY_GESTURE_C;
				break;
			case (int)'E':
				key_data = KEY_GESTURE_E;
				break;
			case (int)'W':
				key_data = KEY_GESTURE_W;
				break;
			case (int)'O':
				key_data = KEY_GESTURE_O;
				break;
			case (int)'M':
				key_data = KEY_GESTURE_M;
				break;
				case (int)'Z':
				key_data = KEY_GESTURE_Z;
				break;
	
			case (int)'V':
				key_data = KEY_GESTURE_V;
				break;
		
			case (int)'S':
				key_data = KEY_GESTURE_S;
				break;
			case (int)'*':	
				key_data = KEY_GESTURE_U;
				break; /* double click */						
			...
			if(key_data != 0){
				...
				//gsl_gesture_status = GE_WAKEUP; //zxw modify for mx1091_hyf_9300 ???????????????????
			...
			printk("Linc gsl_obtain_gesture():tmp_c=%c\n",gsl_gesture_c);
...
static void gsl_hw_init(void)
{
    ...
    #else
            //hwPowerOn(MT6350_POWER_LDO_VGP1, VOL_2800, "TP");
    #endif
...
/*
    ...
    */
	...
	tpd_load_status = 1;
        printk("mtk_gslX68X.c tpd_load_status=%d,return 0\n",tpd_load_status);
...
static void gsl_suspend(struct device *h)
{
	...
	if (tpd_proximity_flag == 1)
	{
	printk("Linc GSL168X In PROXIMITY MODE,Abort Suspend\n");
	    return;
	}
#endif
	gsl_halt_flag = 1;
...
//module_init(gsl_driver_init); //modified by cheguosheng
rootfs_initcall(gsl_driver_init);

...
//    const char *vcc_i2c_name = "vcc_i2c";
    ...
    #ifdef MSG2238A_I2C_ADDR_0X27 //modified by cheguosheng
	  client->addr = 0x27;
	  #else
    client->addr = 0x26;
    #endif
    
	printk("mstar tpd_probe:client->addr=%d\n",client->addr);
...
static int tp_ps_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	return 0;
}

static int tp_ps_flush(void)
{
...
static int  tpd_ps_local_init(void)
{
	...
	ps_ctl.batch = tp_ps_batch;
	ps_ctl.flush = tp_ps_flush;
...
{
    TPD_DMESG("msg2233/msg2238/mstar touch panel driver exit\n");

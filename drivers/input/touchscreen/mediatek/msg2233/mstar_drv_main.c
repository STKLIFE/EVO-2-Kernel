...
u32 SLAVE_I2C_ID_DWI2C = (0x4E>>1); //0x27
#else //original
...
s32 DrvMainTouchDeviceInitialize(void)
{
    ...
    printk("DrvMainTouchDeviceInitialize g_ChipType =0x%x\n ",g_ChipType);
...
//        tsps_assist_register_callback("msg2xxx", &DrvPlatformLyrTpPsEnable, &DrvPlatformLyrGetTpPsData);
    ...
    DrvIcFwLyrGetCustomerFirmwareVersion(&nMajor, &nMinor, &_gFwVersion);
    DBG(&g_I2cClient->dev, "*** %s() nMajor = 0x%x, nMinor = 0x%x***\n", __func__, nMajor, nMinor);
    tpd_firmware_version[0] = (int)(nMajor);
    tpd_firmware_version[1] = (int)(nMinor);

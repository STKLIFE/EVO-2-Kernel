...
***********************************************************************/
 ...
 {	 
    ...
    //added by cheguosheng start
	if(client->addr != 0x38)
	{
		client->addr = 0x38;
		TPD_DMESG("focaltech tpd_probe client->addr=0x%x\n",client->addr);
	}
    //added by cheguosheng end
...
*/
	...
	*/
    ...
    printk("focaltech tpd_probe success\n");
...
***********************************************************************/
 static int tpd_local_init(void)
 {
   	...
   	{
        	printk("focaltech tpd_local_init:fts unable to add i2c driver.\n");
      		return -1;
    	}
    	if(tpd_load_status == 0) 
    	{
       	printk("focaltech tpd_local_init:fts add error touch panel driver.\n");
...
***********************************************************************/
 static int __init tpd_driver_init(void) {
        printk("focaltech tpd_driver_init:MediaTek fts touch panel driver init\n");
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 19))
	tpd_get_dts_info();
#else
	...
	if(tpd_driver_add(&tpd_device_driver) < 0)
       	TPD_DMESG("focaltech tpd_driver_init:add fts driver failed\n");
	 return 0;
 }

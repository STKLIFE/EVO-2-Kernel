...
#if defined (S99_WVGA_MAILETAI_J4001)
#include "icn87xx_s99_wvga_mailetai_j4001.h"
#else
#include "icn87xx_fw_s810_dm_fwvga_gjst_10ch.h"
#endif

#if defined(CUSTOM_TP_PROXIMITY)
...
#if 1//TPD_ICN85XX_SUPPORT_FW_UPDATE
...
#define GESTURE_ENABLE_PROC_FILE      "icn87xx_enable" //linhui
static struct proc_dir_entry *icn85xx_gesture_enable_proc = NULL; 
static int gtp_wakeup_flag = 1; //linhui
#define ICN85XX_GESVAL_PROC_NAME            "icn87xx_gesval"
...
static const struct i2c_device_id icn85xx_tpd_id[] = {{ICN87XX_NAME, 0},{}};
...
#if defined (S99_WVGA_MAILETAI_J4001)
        { 50,   155,  854, 2000, KEY_MENU},
        { 150, 255,  854, 2000, KEY_HOMEPAGE },
        { 250, 999,  854, 2000, KEY_BACK},
        { -1, -1, -1, -1, KEY_UNKNOWN },
#else
        { 0,   155,  854, 2000, KEY_MENU},
        { 156, 255,  854, 2000, KEY_HOMEPAGE },
        { 256, 999,  854, 2000, KEY_BACK},
...
void init_proc_node(void)
{
	...
	icn85xx_proc_entry = proc_create(ICN87XX_ENTRY_NAME, 0666, NULL, &icn85xx_proc_fops);   
...
void uninit_proc_node(void)
{
	kfree(cmd_head.data);
	cmd_head.data = NULL;
	remove_proc_entry(ICN87XX_ENTRY_NAME, NULL);
}
...
static void icn85xx_gesture_handler(void)
{
	...
	icn85xx_trace("++++++TPD_GESTURE_WAKEUP+++++++++++++++\n");
...
static ssize_t icn85xx_gesture_enable_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
    ...
    printk("print hex data icn87xx_enable=0x%x\n", (u8)enable); 
...
static irqreturn_t tpd_eint_interrupt_handler(int irq, void *dev_id)
{
	icn85xx_trace("icn85xx interrupt has been triggered\n");
	tpd_flag = 1;
...
static int  tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	icn85xx_trace("icn85xx tpd_probe addr=0x%x\n",client->addr);
	
	icn85xx_ts = kzalloc(sizeof(*icn85xx_ts), GFP_KERNEL);
	if(!icn85xx_ts)
	{
		icn85xx_trace("icn85xx tpd_probe Alloc icn85xx_ts memory failed.\n");
		return -ENOMEM;
	}
	...
	icn85xx_ts->work_mode = 0;
		...
		if (!icn85xx_i2c_dma_va) {
			icn85xx_trace("icn85xx tpd_probe Allocate DMA I2C Buffer failed!");
			return -1;
		}
		...
		icn85xx_trace("icn85xx %s, TPD icn85xx dma_alloc_coherent success!\n", __FUNCTION__);
#endif


        icn85xx_trace("icn85xx tpd_probe %s,%x\n", __FUNCTION__,i2c_client->addr);
	icn85xx_hw_init();
	err = icn85xx_i2c_test();
	if (err <= 0)
	{
		icn85xx_trace("icn85xx tpd_probe icn85xx_iic_test  failed.\n");
		return -1;
	}
	else
	{
		icn85xx_trace("icn85xx tpd_probe iic communication ok: 0x%x\n", icn85xx_ts->ictype); 
	}

	icn85xx_trace("icn85xx #############%s, tp-probe-00002 \n", __FUNCTION__);
	...
	if((icn85xx_ts->ictype == ICN85XX_WITHOUT_FLASH_85) || (icn85xx_ts->ictype == ICN85XX_WITHOUT_FLASH_86))
	{
		...
		{
			icn85xx_ts->code_loaded_flag = 1;
			icn85xx_trace("icn85xx tpd_probe ICN85XX_WITHOUT_FLASH, update ok\n"); 

		}
		else
		{
			icn85xx_ts->code_loaded_flag = 0;
			icn85xx_trace("icn85xx tpd_probe ICN85XX_WITHOUT_FLASH, update error\n"); 
			return -1;
		}
		...
		icn85xx_trace("icn85xx tpd_probe ICN85XX_WITH_FLASH_85 fwVersion : 0x%x\n", fwVersion);
		icn85xx_trace("icn85xx tpd_probe ICN85XX_WITH_FLASH_85 current version: 0x%x\n", curVersion);
		...
		while(retry > 0)
		{
			if(icn85xx_goto_progmode() != 0)
			{
				icn85xx_trace("icn85xx tpd_probe icn85xx_goto_progmode() != 0 error\n");
				return -1; 
			} 
			icn85xx_read_flashid();
			icn85xx_trace("icn85xx tpd_probe begin to update\n");
			if(R_OK == icn85xx_fw_update(firmware))
			{
				break;
			}
			retry--;
			icn85xx_trace("icn85xx tpd_probe icn85xx_fw_update failed.\n");
		}
#else
    ...
    {
        icn85xx_trace("icn85xx tpd_probe icn85xx_update  87 without flash\n");
        ...
        fwVersion = icn85xx_read_fw_Ver(firmware);
        curVersion = icn85xx_readVersion();
        icn85xx_trace("icn85xx tpd_probe ICN85XX_WITHOUT_FLASH_87 fwVersion : 0x%x\n", fwVersion); 
        icn85xx_trace("icn85xx tpd_probe ICN85XX_WITHOUT_FLASH_87 current version: 0x%x\n", curVersion);
  
        if(R_OK == icn87xx_fw_update(firmware))
        {
            icn85xx_ts->code_loaded_flag = 1;
            icn85xx_trace("icn85xx tpd_probe ICN87XX_WITHOUT_FLASH, update default fw ok\n");
        }
        else
        {
            icn85xx_ts->code_loaded_flag = 0;
            icn85xx_trace("icn85xx tpd_probe ICN87XX_WITHOUT_FLASH, update error\n"); 
        }
     
    }
    else if(icn85xx_ts->ictype == ICN85XX_WITH_FLASH_87)
    {
        icn85xx_trace("icn85xx tpd_probe icn85xx_update 87 with flash\n");
        ...
        fwVersion = icn85xx_read_fw_Ver(firmware);
        curVersion = icn85xx_readVersion();
        icn85xx_trace("icn85xx tpd_probe ICN85XX_WITH_FLASH_87 fwVersion : 0x%x\n", fwVersion); 
        icn85xx_trace("icn85xx tpd_probe ICN85XX_WITH_FLASH_87 current version: 0x%x\n", curVersion); 
            ...
            {
                icn85xx_ts->code_loaded_flag = 1;
                icn85xx_trace("icn85xx tpd_probe ICN87XX_WITH_FLASH, update default fw ok\n");
            }
            else
            {
                icn85xx_ts->code_loaded_flag = 0;
                icn85xx_trace("icn85xx tpd_probe ICN87XX_WITH_FLASH, update error\n"); 
            }    
            ...
            if(fwVersion > curVersion)
            {
                ...
                while(retry > 0)
                {
                    ...
                    retry--;
                    icn85xx_trace("icn85xx tpd_probe icn87xx_fw_update failed.\n");   
                }
            }
        #endif
    }
     ...
     icn85xx_trace("icn85xx tpd_probe tpd use default setting,screen_max_x=%x,screen_max_y=%x, \n",  icn85xx_ts->screen_max_x,  icn85xx_ts->screen_max_y); 
	...
	init_proc_node();
		...
		{
			icn85xx_trace("icn85xx tpd_probe attach fail = %d\n", errpro);
		}
...
//****************************************************//
      icn85xx_trace("icn85xx tpd_probe create kthread_run \n");
	thread = kthread_run(touch_event_handler, 0, TPD_DEVICE);
	if(IS_ERR(thread))
	{
		retval = PTR_ERR(thread);
		icn85xx_trace(TPD_DEVICE " icn85xx tpd_probe failed to create kernel thread: %d\n", retval);
	}
	...
	icn85xx_esd_workqueue = create_workqueue("icn85xx_esd");
		...
		icn85xx_ts->clk_tick_cnt = 3 * HZ;		// HZ: clock ticks in 1 second generated by system
	icn85xx_trace("icn85xx tpd_probe Clock ticks for an esd cycle: %d", icn85xx_ts->clk_tick_cnt);
	...
	if (!icn85xx_ts->esd_running)
		...
		icn85xx_trace("icn85xx tpd_probe Esd started");
		queue_delayed_work(icn85xx_esd_workqueue, &icn85xx_esd_event_work, icn85xx_ts->clk_tick_cnt);
	}
	else 
	{                        
	...
	icn85xx_trace("icn85xx tpd_probe Touch Panel Device Probe over,tpd_load_status=%d\n",tpd_load_status);
...
static int tpd_local_init(void)
{
	...
	icn85xx_trace("icn85xx tpd_local_init Device Tree get regulator!");
        #if 0 //modified by cheguosheng
        tpd->reg = regulator_get(tpd->tpd_dev, "vtouch1");
        #else //original
   ...
   //  icn85xx_trace("ICN85XX I2C Touchscreen Driver (Built %s @ %s)\n", __DATE__, __TIME__);
	...
	if (tpd_load_status == 0) {
		printk("icn87xx tpd_local_init add error touch panel driver.tpd_load_status==0\n");
...
#ifdef TPD_PROXIMITY
static unsigned char PS_BK_flag=0;
#endif
...
//=======================================Gesture  CODE===============================
#ifdef TPD_PROXIMITY
	if (tpd_proximity_flag == 1 && PS_BK_flag ==0) 
	{
		return;
	}
#endif
#ifdef TPD_GESTURE_WAKEUP
    suspend_status=0;
    icn85xx_trace("==icn85xx_ts_resume==  gesture_id=0x%x\n", gesture_id);
#endif
...
//module_init(tpd_driver_init);   //modified by cheguosheng
rootfs_initcall(tpd_driver_init);

...
static imgsensor_info_struct imgsensor_info = {
    ...
    .checksum_value = 0x5dff3ae2, // 0xc96d86d7        //checksum value for Camera Auto Test
        ...
        do {
            *sensor_id = (return_sensor_id()&0xffef)+sensor_index;
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("gc2385mipi_Sensor get_imgsensor_id i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("gc2385mipi_Sensor get_imgsensor_idRead sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = (return_sensor_id()&0xffef)+sensor_index;
            if (sensor_id == imgsensor_info.sensor_id) {
                printk("gc2385mipi_Sensor open i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            printk("gc2385mipi_Sensor open Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)

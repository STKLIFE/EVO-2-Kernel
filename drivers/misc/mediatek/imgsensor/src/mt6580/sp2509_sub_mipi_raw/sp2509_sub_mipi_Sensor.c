...
#define PFX "SP2509SUB_camera_sensor"
#define LOG_1 LOG_INF("SP2509SUB,MIPI 1LANE\n")
...
//#define DEBUG_SENSOR_SP2509SUB
#ifdef DEBUG_SENSOR_SP2509SUB	//sp_zze
#include <asm/uaccess.h>
#include <linux/fs.h>
#include <linux/string.h>

#define SP2509SUB_SD_FILE_PATH					"/mnt/sdcard/SP2509SUB_sd"
#define SP2509SUB_SD_PREVIEW					"/mnt/sdcard/SP2509SUB_sd_preview"
#define SP2509SUB_SD_CAPTURE					"/mnt/sdcard/SP2509SUB_sd_capture"
//#define SP2509SUB_SD_VIDEO						"/mnt/sdcard/SP2509SUB_sd_video"
...
static U32 SP2509SUB_initialize_from_t_flash(char *file_path)
{
	U8 *curr_ptr = NULL;
	U8 *tmp = NULL;
	U32 file_size = 0;
	...
	SP2509SUB_init_regs_struct SP2509SUB_reg;
	...
	while (curr_ptr < (data_buff + file_size)) {
		...
		if (!strcmp((const char *)func_ind, "REG")) {		/* REG */
			curr_ptr += 4;				/* Skip "REG{" */
			SP2509SUB_reg.addr = sp_strtoul((const char *)curr_ptr, &tmp, 16);
			curr_ptr = tmp;
			curr_ptr ++;	/* Skip "," */
			SP2509SUB_reg.val = sp_strtoul((const char *)curr_ptr, &tmp, 16);
			curr_ptr += 2;	/* Skip "};" */
			
			//init
			SP_I2C_WRITE(SP2509SUB_reg.addr, SP2509SUB_reg.val);
			regs_total++;
		} else if (!strcmp((const char *)func_ind, "DLY")) {	/* DLY */
			curr_ptr += 4;				/* Skip "DLY(" */
			SP2509SUB_reg.val = sp_strtoul((const char *)curr_ptr, &tmp, 10);
			curr_ptr = tmp;

			SP_DELAY(SP2509SUB_reg.val);
		}
...
static kal_uint16 set_gain(kal_uint16 gain)
{
   	 ...
   	 {   
   	    	 ...
   	    	 if(iReg<=0x10)
   	    	 {
   	    	    	 ...
   	    	    	 write_cmos_sensor(0x01, 0x01);
                      LOG_INF("SP2509SUBMIPI_SetGain = 16");
   	    	 }
   	    	 else if(iReg>= 0xa0)//gpw
   	    	 {
   	    	    	 ...
   	    	    	 write_cmos_sensor(0x01, 0x01); 
                      LOG_INF("SP2509SUBMIPI_SetGain = 160"); 
	        }        	
   	    	 else
   	    	 {
   	    	    	write_cmos_sensor(0xfd, 0x01);
   	    	    	...
   	    	    	write_cmos_sensor(0x01, 0x01);
			LOG_INF("SP2509SUBMIPI_SetGain = %d",iReg);		 
	       }	
   	 }	
   	 else
...
#ifdef DEBUG_SENSOR_SP2509SUB
		if(read_from_extern_flash(SP2509SUB_SD_FILE_PATH)) {
			SP_LOG("init from t!\n");
			SP2509SUB_initialize_from_t_flash(SP2509SUB_SD_FILE_PATH);
		} else {
			sensor_init();
		}
#else  
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	#if 0 //def DEBUG_SENSOR_SP2509SUB
		if(read_from_extern_flash(SP2509SUB_OTHER_DEBUG_PATH)) {
			printk("sp_zze: other debug!\n");
			SP2509SUB_other_debug(SP2509SUB_OTHER_DEBUG_PATH);
		}
	#endif
...
UINT32 SP2509_SUB_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    SP2509SUBMIPISensorInit    */

...
//For hardware information display
//add by frank begin 
#ifdef CONFIG_JZHK_HARDWARE_INFOMATION
char mtkfb_frontcam_name[256] = {0};
char mtkfb_backcam_name[256] = {0};
EXPORT_SYMBOL(mtkfb_frontcam_name);
EXPORT_SYMBOL(mtkfb_backcam_name);
#endif
//add by frank end  
/* Camera Infos */
		...
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			if (0 != (g_CurrentSensorIdx & g_invokeSocketIdx[i])) {
		        ...
		        if(ret == ERROR_NONE)
		        {
					if(i == 0)
					{
						strcpy(mtkfb_backcam_name,g_invokeSensorNameStr[i]);
					}
					else
					{
						strcpy(mtkfb_frontcam_name,g_invokeSensorNameStr[i]);
					}
				}
				#endif
		        //add by frank end  

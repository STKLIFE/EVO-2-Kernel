...
#include "sp250a_sub_mipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "SP250A_SUB__camera_sensor"
#define LOG_1 LOG_INF("SP250A_SUB_,MIPI 1LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = SP250A_SUB_MIPI_SENSOR_ID,
...
#define DEBUG_SENSOR
#ifdef DEBUG_SENSOR
	#define SP250A_SUB_MIPI_OP_CODE_INI		0x00		/* Initial value. */
	#define SP250A_SUB_MIPI_OP_CODE_REG		0x01		/* Register */
	#define SP250A_SUB_MIPI_OP_CODE_DLY		0x02		/* Delay */
	#define SP250A_SUB_MIPI_OP_CODE_END		0x03		/* End of initial setting. */
	...
	} SP250A_SUB_MIPI_initial_set_struct;

	SP250A_SUB_MIPI_initial_set_struct SP250A_SUB_MIPI_Init_Reg[1000];
 ...
 static u32 strtol(const char *nptr, u8 base)
{
 ...
 static u8 SP250A_SUB_MIPI_Initialize_from_T_Flash(void)
{
	...
	fp = filp_open("/mnt/sdcard/sp250a_sub__sd", O_RDONLY , 0); 
	if (IS_ERR(fp)) 
	{ 
	...
	while (curr_ptr < (data_buff + file_size))
	{
		...
		if (strcmp((const char *)func_ind, "REG") == 0)		/* REG */
		{
			...
			SP250A_SUB_MIPI_Init_Reg[i].op_code = SP250A_SUB_MIPI_OP_CODE_REG;
			
			SP250A_SUB_MIPI_Init_Reg[i].init_reg = strtol((const char *)curr_ptr, 16);
			curr_ptr += 5;	/* Skip "00, 0x" */
		
			SP250A_SUB_MIPI_Init_Reg[i].init_val = strtol((const char *)curr_ptr, 16);
			curr_ptr += 4;	/* Skip "00);" */
		...
		else 									/* DLY */
		{
			...
			curr_ptr += 4;	
			SP250A_SUB_MIPI_Init_Reg[i].op_code = SP250A_SUB_MIPI_OP_CODE_DLY;
			
			SP250A_SUB_MIPI_Init_Reg[i].init_reg = 0xFF;
			SP250A_SUB_MIPI_Init_Reg[i].init_val = strtol((const char *)curr_ptr,  10);	/* Get the delay ticks, the delay should less then 50 */
		}
		i++;
	...
	SP250A_SUB_MIPI_Init_Reg[i].op_code = SP250A_SUB_MIPI_OP_CODE_END;
	SP250A_SUB_MIPI_Init_Reg[i].init_reg = 0xFF;
	SP250A_SUB_MIPI_Init_Reg[i].init_val = 0xFF;
	i++;
	//for (j=0; j<i; j++)
		//printk(" %x  ==  %x\n",sp250a_sub_MIPI_Init_Reg[j].init_reg, sp250a_sub_MIPI_Init_Reg[j].init_val);

	/* Start apply the initial setting to sensor. */
	#if 1
	for (j=0; j<i; j++)
	{
		if (SP250A_SUB_MIPI_Init_Reg[j].op_code == SP250A_SUB_MIPI_OP_CODE_END)	/* End of the setting. */
		{
			break ;
		}
		else if (SP250A_SUB_MIPI_Init_Reg[j].op_code == SP250A_SUB_MIPI_OP_CODE_DLY)
		{
			msleep(SP250A_SUB_MIPI_Init_Reg[j].init_val);		/* Delay */
		}
		else if (SP250A_SUB_MIPI_Init_Reg[j].op_code == SP250A_SUB_MIPI_OP_CODE_REG)
		{
		
			write_cmos_sensor(SP250A_SUB_MIPI_Init_Reg[j].init_reg, SP250A_SUB_MIPI_Init_Reg[j].init_val);
			printk("%x = %x\n",SP250A_SUB_MIPI_Init_Reg[j].init_reg,SP250A_SUB_MIPI_Init_Reg[j].init_val);
		}
		else
		{
...
extern u32 sensor_index ;
static kal_uint32 return_sensor_id(void)
{
       write_cmos_sensor(0xfd, 0x00);
	return (((read_cmos_sensor(0x02) << 8) | read_cmos_sensor(0x03)) + 0X01);
}
...
static kal_uint16 set_gain(kal_uint16 gain)
{
   	 ...
   	 {   
   	    	 ...
   	    	 if(iReg<=0x10)
   	    	 {
   	    	    	 ...
   	    	    	 write_cmos_sensor(0x01, 0x01);
                      LOG_INF("SP250A_SUB_MIPI_SetGain = 16");
   	    	 }
   	    	 else if(iReg>= 0xa0)//gpw
   	    	 {
   	    	    	 ...
   	    	    	 write_cmos_sensor(0x01, 0x01); 
                      LOG_INF("SP250A_SUB_MIPI_SetGain = 160"); 
	        }        	
   	    	 else
   	    	 {
   	    	    	write_cmos_sensor(0xfd, 0x01);
   	    	    	...
   	    	    	write_cmos_sensor(0x01, 0x01);
			LOG_INF("SP250A_SUB_MIPI_SetGain = %d",iReg);		 
	       }	
   	 }	
   	 else
...
static void sensor_init(void)
{
	...
	if(fromsd == 1)//是否从SD读取//gepeiwei   120903
		...
		SP250A_SUB_MIPI_Initialize_from_T_Flash();//从SD卡读取的主要函数
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        spin_unlock(&imgsensor_drv_lock);
        do {
            *sensor_id = return_sensor_id()+sensor_index;
    ...
    if (imgsensor_info.sensor_id != sensor_id)
      ...
      //   loff_t pos = 0; 
	   //  static char buf[10*1024] ;
	    ...
	    {
		    fromsd = 1;
	          //sp250a_sub_MIPI_Initialize_from_T_Flash();
	          printk("gpww read ok!\n");
...
UINT32 SP250A_SUB_MIPISensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    SP250A_SUB_MIPISensorInit    */

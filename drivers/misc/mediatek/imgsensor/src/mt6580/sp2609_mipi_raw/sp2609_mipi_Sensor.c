        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("sp2609 get_imgsensor_id i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("sp2609 get_imgsensor_id Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            if (sensor_id == imgsensor_info.sensor_id) {
                printk("sp2609 open Expect id: 0x%x,read sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            printk("sp2609 open Read sensor id fail, Expect id:0x%x, read sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
    ...
    if (imgsensor_info.sensor_id != sensor_id)
      ...
      struct file *fp; 
	     ...
	     static char buf[10*1024] ;
          ...
          if (IS_ERR(fp)) { 
		     fromsd = 0;   
		     printk("sp2609 open open file error\n");
		     //return 0;
           } 	

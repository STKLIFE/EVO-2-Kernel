...
#include "gc2385submipi_Sensor.h"
...
kal_bool GC2385SUBDuringTestPattern = KAL_FALSE;

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC2385SUB_MIPI_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        spin_unlock(&imgsensor_drv_lock);
        do {
			 printk("gc2385_sub_mipi_raw get_imgsensor_id i2c write id: 0x%x, sensor id: 0x%x\n", return_sensor_id()&0xffef,sensor_index);
            *sensor_id = (return_sensor_id()&0xffef)+sensor_index;
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("gc2385_sub_mipi_raw get_imgsensor_id i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				
                return ERROR_NONE;
            }
            printk("gc2385_sub_mipi_raw get_imgsensor_id Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        spin_unlock(&imgsensor_drv_lock);
        do {
		printk("gc2385_sub_mipi_raw open i2c write id: 0x%x, sensor id: 0x%x\n", return_sensor_id()&0xffef,sensor_index);
            sensor_id = (return_sensor_id()&0xffef)+sensor_index;
            if (sensor_id == imgsensor_info.sensor_id) {
                printk("gc2385_sub_mipi_raw open i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            printk("gc2385_sub_mipi_raw open Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
    ...
    spin_unlock(&imgsensor_drv_lock);
    GC2385SUBDuringTestPattern = KAL_FALSE; 
...
UINT32 GC2385SUBMIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{

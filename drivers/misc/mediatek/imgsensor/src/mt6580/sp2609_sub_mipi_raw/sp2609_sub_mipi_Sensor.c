...
#include "sp2609_sub_mipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "SP2609SUB_camera_sensor"
#define LOG_1 LOG_INF("SP2609SUB,MIPI 1LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = SP2609_SUB_SENSOR_ID,

    .checksum_value = 0x2e18b052,        //checksum value for Camera Auto Test
...
static void write_shutter(kal_uint16 shutter)
{
        ...
        write_cmos_sensor(0x01, 0x01); 
        LOG_INF("sp2609sub11111-------shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);
        
        /*add blacklevel log for sp2609sub*/
}
...
static kal_uint16 set_gain(kal_uint16 gain)
{
   	 ...
   	 {   
   	    	 ...
   	    	 if(iReg<=0x10)
   	    	 {
   	    	    	 ...
   	    	    	 write_cmos_sensor(0x01, 0x01);
                      LOG_INF("SP2609SUBMIPI_SetGain = 16");
   	    	 }
   	    	 else if(iReg>= 0xf8)//huazai
   	    	 {
   	  					...
   	  					write_cmos_sensor(0x01, 0x01); 
                      LOG_INF("SP2609SUBMIPI_SetGain = 160"); 
	        }        	
   	    	 else
   	    	 {
   	    	    	write_cmos_sensor(0xfd, 0x01);
   	    	    	...
   	    	    	write_cmos_sensor(0x01, 0x01);
			LOG_INF("sp2609sub11111-------SP2609SUBMIPI_SetGain ireg = 0x%x,gain = 0x%x\n",iReg,gain);		 
	       }	
   	 }	
   	 else
...
static void sensor_init(void)
{
	...
	if(fromsd == 1)//是否从SD读取//gepeiwei   120903
		...
		sp2609subMIPI_Initialize_from_T_Flash();//从SD卡读取的主要函数
        ...
        do {
            ...
            *sensor_id = *sensor_id+1;
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("sp2609_sub get_imgsensor_id i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("sp2609_sub get_imgsensor_id Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            sensor_id++;
            if (sensor_id == imgsensor_info.sensor_id) {
                printk("sp2609_sub open id: 0x%x,read sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            printk("sp2609_sub open Read sensor id fail, Expect id:0x%x, read sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
    ...
    if (imgsensor_info.sensor_id != sensor_id)
      ...
      struct file *fp; 
	     ...
	     static char buf[10*1024] ;
          ...
          fp = filp_open("/mnt/sdcard/sp2609sub_sd", O_RDONLY , 0); 
          if (IS_ERR(fp)) { 
		     fromsd = 0;   
		     printk("sp2609_sub open open file error\n");
		     //return 0;
           } 	
	    ...
	    {
		    fromsd = 1;
	          //sp2609subMIPI_Initialize_from_T_Flash();
	          printk("sp2609_sub open read ok!\n");
...
UINT32 SP2609SUBMIPISensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    sp2609subMIPISensorInit    */

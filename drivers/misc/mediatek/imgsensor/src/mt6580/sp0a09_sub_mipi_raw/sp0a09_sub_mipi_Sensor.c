...
#include "sp0a09_sub_mipi_Sensor.h"
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = SP0A09_SUB_SENSOR_ID,
        ...
        do {
			...
			*sensor_id = *sensor_id + 1;
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("sp0a09_sub get_imgsensor_id i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("sp0a09_sub get_imgsensor_id Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			sensor_id++;
            if (sensor_id == imgsensor_info.sensor_id) {
                printk("sp0a09_sub open i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            printk("sp0a09_sub open Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
...
{
    printk("cgs sp0a09_sub_mipi_raw enable: %d\n", enable);
...
UINT32 SP0A09_SUB_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{

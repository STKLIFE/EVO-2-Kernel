...
//static struct i2c_board_info      msa_i2c_boardinfo = { I2C_BOARD_INFO(MSA_DRV_NAME, 0x26)};//MSA_I2C_ADDR>>1) };

//static struct i2c_client *msa_i2c_client;
static struct msa_i2c_data *obj_i2c_data;
...
static struct acc_hw *msa_hw = &accel_cust;
...
static int msa_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	return 0;
}

static int msa_flush(void)
{
...
static int msa_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
   ...
   printk("msa_probe\n");
 	if(client->addr != 0x26)
	{
		client->addr = 0x26;//water force i2c
...
{    
	//const char *name = "mediatek,msa301";
	printk("msa_init\n");
    /*
	msa_hw = check_accel_dts_func(name, msa_hw);
	if(msa_hw == NULL)
	{
			MI_ERR("msa_hw is null");
			return 0;
	}
*/

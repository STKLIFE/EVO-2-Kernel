...
void oam_init(void)
{
	...
	if (bat_is_charger_exist() == KAL_TRUE) {
		...
		//add by liuhui 20190323 start
		//modify for when the machine power off with no capacity, then charging with power off , battery percentage dump to 17% from 0%
		#ifdef CONFIG_JZHK_CHARGING_OPTIMIZE_WITH_LOW_POWER
		if((gFG_capacity_by_v > 6)&&(gFG_capacity_by_v < 20)&&(vbat_capacity < 3450))
		{
			bm_print(BM_LOG_FULL,"[oam_init] fg_vbat=(%d), vbat=(%d), set fg_vat as vat\n",gFG_voltage, g_booting_vbat);
			gFG_voltage = g_booting_vbat;
			gFG_capacity_by_v = vbat_capacity;
		}
		#endif
		//add by liuhui 20190323 end
	}
